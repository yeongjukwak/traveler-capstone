...
$(document).ready(function(){
        ...
        if(name === marker._marker_data.options.title){
          ...
          if(!(popups.length === 0)){
            for(let j=0; j<popups.length; j++){
              if(popups[j].spot === name){
                if(popups[j].visibility){
                  popups[j].popup.setMap(null);
                  popups.splice(j, 1);
                  return;
                }
              }
  ...
  function ctp(data, marker){
    let content = `<div class='m-pop'>`+
                    ...
                    `<div class='info-box'>`+
                        ...
                        `<a href='${data.viewUrl}' target='_blank' class='link'><img src='./resources/map/images/popup/3d-view.svg' alt='3D뷰'> View</a>`+
                        `</p>`+
                        `<ul class='ul-info'>`+
                          `<li class='li-addr'>`+
                        ...
                        `</ul>`+
                    `</div>`+
                    `<a href='javascript:void(0)' name='${data.spot}' class='btn-close'></a>`+
                  `</div>`;
    ...
    marker.addListener("click", function(event) {
      ...
      for(let i=0; i<popups.length; i++){
        if(popups[i].spot === data.spot){
          if(popups[i].visibility){
            return;
          }
        }
      }
      ...
      popups.push({spot : data.spot, visibility : true, popup : infoWindow});
    });
  }
  ...
  $(document).on("click", ".btn-close", function(){
    ...
    for(let i=0; i<popups.length; i++){
      if(popups[i].spot === name){
        popups[i].popup.setMap(null);
        popups.splice(i, 1);
        break;
      }
    }
